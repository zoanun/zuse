...
/** 自动会话的文件格式。命名存档维持 v1 不动,这里是带元数据的 v2。 */
...
/** cwd → 单个安全目录段(对齐 CC 的 projects 目录编码;盘符冒号、斜杠都归一成 -)。 */
...
export async function listAutoSessions(cwd: string, limit = 10): Promise<SessionMeta[]> {
  ...
  for (const f of files) {
    try {
      ...
      if (record.version !== 2 || !Array.isArray(record.messages)) continue
      metas.push({
...
export async function loadAutoSession(
  cwd: string,
  id: string,
): Promise<{ conversation: Conversation; id: string; createdAt: string }> {
  ...
  if (record.version !== 2) throw new Error(`不支持的会话格式 version: ${String(record.version)}`)
  const conversation = Conversation.fromJSON({
    version: 1,
    messages: record.messages,
    totalUsage: record.totalUsage,
  })
  return { conversation, id, createdAt: record.createdAt }
}
