...
| M3 | skill 管理 | 列出已加载 skill、查看内容/触发条件、启用/禁用；**Web UI 内手动新增 skill**（表单：名称 / description / 触发条件 / 正文 → 落盘为 SKILL.md，即时被 skill 系统识别）；对用户/项目自建 skill 完整 CRUD（新增/编辑/删除）。插件来源的 skill 只读 + 启停（归插件管） | F3 |
